...
let
  ...
  makeInstall = source:
    let vsix = makeVsix source;
    in "${pkgs.vscode}/bin/code --user-data-dir \"$NIX_BUILD_TOP/user-data\" --extensions-dir \"$out/share/extensions\" --install-extension \"${vsix}\"";
in stdenv.mkDerivation {
  ...
  dontStrip = true;
  installPhase = ''
    mkdir -p $out/{bin,share/extensions}
